...
因为很多人都头疼单字的识别码，因此在本方案的词库中对所有包含识别码的单字进行了单独编码，称基为次全码单字。例如「吾」的全码是`gkf`，但在输入`gk`时即可被检索到，而且不会影响`gkf`全码。这样处理的结果就是候选栏中的重码变多了，但是，在首选项位置，固定的简码是不变的，喜欢打全码的则可以忽略其他候选项，依旧可以使用识别码能精准定位到最终结果。而对于一时想不起识别码的单字，只要拆出完整字根，它就一定会出现在候选栏里，这种确信的结果对于某些结构不明确的字来说，大大减轻了大脑从字根拆解到结构识别的能量消耗。
